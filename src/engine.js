  ...
  if (!finalPurifier) {
    ...
    finalPurifier.addHook('uponSanitizeAttribute', (node, data) => {
      if (data.attrName === 'class') {
        ...
        const safe = original.filter(name => !reservedUiClasses.has(name) && !name.startsWith('pdf-'));
